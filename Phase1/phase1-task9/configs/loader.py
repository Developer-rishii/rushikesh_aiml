...
class Config:
    def __init__(self, raw: dict, config_path: Path):
        ...
        self.default_params = m.get("default_params", {})

        s = raw["search"]
        self.search_strategy = s["strategy"]
        self.param_grid = s["param_grid"]
        self.cv_folds = s["cv_folds"]
        self.scoring = s["scoring"]
        self.n_random_iter = s.get("n_random_iter", 20)
    ...
    def __repr__(self):
        return f"Config(model={self.model_name}, strategy={self.search_strategy}, seed={self.seed})"
...
def load_config(config_path: Path = None) -> Config:
    ...
    required = {"seed", "data", "preprocessing", "model", "search", "logging"}
