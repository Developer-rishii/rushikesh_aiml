"""
evaluation/metrics.py — single source of metric computation, called with
validation/test data only.
"""
...
def evaluate_pipeline(fitted_pipeline, X, y, metric_names) -> dict:
    proba = fitted_pipeline.predict_proba(X)[:, 1]
    pred = fitted_pipeline.predict(X)
    return compute_metrics(y, proba, pred, metric_names)
